...
\bottomrule
\end{tabular}
\caption{Equilibrium multiplicity in the three-round auction. Each row shows two profiles, each an $\varepsilon$-Nash equilibrium, that give the toehold-holder the same value but deter the rival with very different probability. Certified $\varepsilon$ by row: $2.4\times10^{-6}$ at $\theold=0$, $6.4\times10^{-6}$ at $\theold=0.15$, and $7.6\times10^{-5}$ at $\theold=0.05$, where the passive profile is the slower of the pair (its partner is at $4.3\times10^{-6}$). Values agree to four decimals except at $\theold=0.05$, where they differ by $6\times10^{-5}$, less than that row's $\varepsilon$, so the two values are not distinguishable at the precision we certify. Deterrence is not identified by the toehold. Regenerate with \texttt{sequential\_general\_sum.py certify}, or in one second from the committed per-restart data with \texttt{certificate\_from\_selection\_csv}.}
\label{tab:multiplicity}
\end{table}

\paragraph{Why, mechanically.} The forced-opening profit curve shows both conducts are self-consistent (Figure~\ref{fig:mechanism}). The cleanest case is the one that carries the second finding. At $\theold=0$ the two equilibria are a rival that folds with probability $0.333$ and a rival that essentially never folds ($3\times10^{-5}$). Against the first, committing to the jump at $1.125$ rather than opening at the bottom of the grid raises the toehold-holder's profit by $179\%$; against the second it raises it by exactly nothing, and the curve peaks at the cheapest opening. So ``I preempt because you fold, you fold because I preempt'' and ``I open low because you will not fold, you will not fold because I open low'' are both equilibria. This is coordination-style multiplicity, not solver failure.

The same pattern appears wherever we can test it, and we can test it less often than we would like. Against the most-deterring equilibrium rival the jump premium is large at every toehold, ranging from $31\%$ at $\theold=0.15$ to $179\%$ at $\theold=0$. Against the least-deterring one it collapses to $2.6\%$ and $5.3\%$ at $\theold=0.10$ and $0.15$, where that rival folds with probability $0.17$ and $0.22$. But at $\theold=0.05$ and $\theold=0.20$ the least-deterring equilibrium we find is not a passive rival at all: it still folds with probability $0.25$ and $0.33$, and the premium against it stays at $77\%$ and $25\%$. Those two toeholds therefore do not test the contrast; only $\theold=0$ exhibits a genuinely non-folding equilibrium rival. As a control, against a \emph{uniform}, unbluffable rival the forced-opening curve slopes straight down with a zero jump premium at every toehold: a jump bid buys deterrence, and deterrence is worthless against a rival that cannot be deterred. Premia here are measured against the profit of opening at the bottom of the grid; the released data reports that normalization and the peak-relative one in separate columns.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{../results/seq_preemption_incentive.png}
\caption{Forced-opening best-response profit for the toehold-holder as a function of the opening bid it commits to, with every later decision best-responding. Left: against the most-deterring equilibrium rival at each toehold, profit peaks at the preemptive jump. Middle: against the least-deterring equilibrium rival. Only the $\theold=0$ curve there is a genuinely non-folding rival, and it is the one that peaks at the cheapest opening; at the other toeholds the least-deterring equilibrium still folds with probability $0.17$ to $0.33$, and its curve still peaks at the jump, which is what the panel subtitles report. Right, the control: against a rival bidding at random, which cannot be deterred, profit slopes straight down with no jump premium at any toehold. A jump bid only pays when it can scare a rival off, which is why both the preempting and the non-preempting conduct are self-consistent equilibria. The forced-opening best response is exact; the two equilibrium rivals are solver output, and in the released version of this figure they are default-budget solves that stopped at NashConv $10^{-4}$ rather than probe-precision ones (see Section~\ref{sec:limitations}). Only the control panel is solver-free.}
\label{fig:mechanism}
\end{figure}
...
\emph{Grid robustness is not established.} We had hoped to report that the multiplicity survives grid refinement. It does not survive the test, in the weaker sense that the test does not run: in the released refinement study only the base nine-level grid reaches the acceptance bound for most restarts. At seven levels five of eighteen solves are accepted and none of them exhibits the low-deterrence conduct; at thirteen levels one of eighteen is accepted. So the multiplicity is exhibited at one grid resolution, and whether it is a discretization artifact is neither established nor refuted. The finer cells are the slowest solves in the suite, at twelve to twenty hours each, and are left to future compute. The headline does not rest on this study, which rests instead on the rounds sweep at $\varepsilon\sim10^{-6}$, but the gap is real and we do not want it read as robustness.

\emph{The mechanism figure lags the tables.} The rival policies behind Figure~\ref{fig:mechanism} in the released artifacts are default-budget solves at NashConv $10^{-4}$, not probe-precision ones, and at two toeholds the least-deterring equilibrium found is not a passive rival at all. The forced-opening construction itself is exact and the control panel is solver-free, so the mechanism reading is sound where a passive rival exists; the certified re-run of that study is wired into \texttt{certify} but its output is not what the committed figure shows.
