...
We ask whether the second effect is there once the contest is modeled as several rounds of escalating bids rather than the single exchange classical models assume. Building on our companion study of sealed auctions \citep{naboulsi2026diligence}, we model a takeover fight in which two bidders take turns, each able to jump to any higher price or walk away, each holding private due-diligence findings about a company worth the same unknown amount to both. We solve it to an accuracy a referee can check, and price an aggressive opening with a calculation that is exact given the rival it is measured against, so it adds no error of its own.

Three things follow, and together they take the deterrence argument apart. In the contest we certify, the auction pins down what the toehold-holder earns but not how it behaves: the same contest supports a bidder who opens aggressively against a rival who folds, and one who opens cheaply against a rival who does not, with the same profit either way and nothing in the game to choose between them. Preemptive bidding survives when the toehold is removed entirely, so it is a feature of bidding in public and in turns, not something a stake buys. And the tidy relationship between toehold size and a rival's willingness to quit, the centerpiece of the classical account, holds only in a contest cut short after one round. Give it a genuine second round and the relationship stops responding; by the third it is gone.

So the two reasons to buy a toehold do not survive equally. The profit reason does, everywhere we look. The deterrence reason does not, which offers a reason from inside the model for why toeholds may be rarer than theory expects, alongside the usual explanations about disclosure and market impact. There is a warning here too for anyone computing economics out of a game solver: run ours once and it returns a confident figure for what a preemptive bid is worth, run it again from a different start and it returns a different one, equally converged, on the same auction. The figure was never a property of the auction.

We also report which solvers can handle contests of this shape. A general-purpose reinforcement-learning method reaches a near-exact solution where the standard baseline from that literature fails to converge at all, while the exact specialist solvers stay faster wherever the game can still be enumerated and only run out when it cannot. All games, solvers, and experiments are released.
